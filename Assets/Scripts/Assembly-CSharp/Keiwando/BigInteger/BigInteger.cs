...
namespace Keiwando.BigInteger
{
	public class BigInteger
	{
		...
		{
			return m_digits.GetDataAsString();
		}
		...
		public string ToString(int radix)
		{
			...
			while (bigInteger.m_digits.DataUsed > 1 || (bigInteger.m_digits.DataUsed == 1 && bigInteger.m_digits[0] != 0))
			{
				...
				Divide(bigInteger, rightSide, out quotient, out remainder);
		...
		public static BigInteger operator +(BigInteger leftSide, BigInteger rightSide)
		{
			...
			{
				long num3 = (long)leftSide.m_digits[i] + (long)rightSide.m_digits[i] + num2;
				num2 = num3 >> DigitsArray.DataSizeBits;
				digitsArray[i] = (uint)(num3 & DigitsArray.AllBits);
			}
		...
		public static BigInteger operator /(BigInteger leftSide, BigInteger rightSide)
		{
			...
			return (isNegative2 == isNegative) ? quotient : (-quotient);
		}
		...
		public static BigInteger operator >>(BigInteger leftSide, int shiftCount)
		{
			...
			if (leftSide.IsNegative)
			{
				...
				for (int i = 0; i < DigitsArray.DataSizeBits; i++)
				{
					...
					DigitsArray digitsArray3 = (digitsArray2 = digitsArray);
					int index;
					int index2 = (index = digitsArray.DataUsed - 1);
					uint num3 = digitsArray2[index];
					digitsArray3[index2] = num3 | num2;
					num2 >>= 1;
				}
				digitsArray.DataUsed = digitsArray.Count;
			}
